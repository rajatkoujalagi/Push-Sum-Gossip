  ...
  var len=0
   def receive ={
     ...
     case Rumour=>
      ...
      context.system.scheduler.schedule(0 milliseconds, 250 milliseconds,self,Fail)
      //println("Actor "+id+" is now sending the rumour")
       }
       else if(counter>=10){
         /*for(i<-0 to len-1){
           neighbours(i) ! Dead(self)
         }
         println()*/
     ...
     case Dead(a)=>
       ...
       //println("New neighbour list for "+id+" is "+neighbours)
       len=neighbours.length
     case Fail=>
       failCount+=1
       //println(s"Fail count for $id is now $failCount")
  ...
  def receive={
    ...
    case HeardRumour=>
      ...
      if(counter==numNodes){
        ...
        println("Time taken to converge to gossip "+(endTime-startTime))
